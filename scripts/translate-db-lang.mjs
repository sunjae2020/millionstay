...
//
// Usage:
//   node scripts/translate-db-lang.mjs <env-file> <lang> <out.sql> [--identical]
//   e.g. node scripts/translate-db-lang.mjs artifacts/api-server/.env vi /tmp/vi.sql
//
// --identical also re-translates rows whose stored value is byte-identical to
// the English source. Those are usually a half-finished seeding run rather than
// a deliberate choice; acronyms and brand names simply come back unchanged.
...
const [ENV_FILE, LANG, OUT] = process.argv.slice(2).filter((a) => !a.startsWith("--"));
const REFILL_IDENTICAL = process.argv.includes("--identical");
...
WHERE e.lang = 'en' AND e.value <> '' AND (t.id IS NULL OR btrim(t.value) = ''${REFILL_IDENTICAL ? " OR t.value = e.value" : ""});
`;
...
const missingByKey = new Map(missing.map((m) => [m.key, m.en]));
const lines = [
  "BEGIN;",
...
for (const { key } of missing) {
  ...
  lines.push(
    ...
    REFILL_IDENTICAL
      ? `  WHERE btrim(translations.value) = '' OR translations.value = ${q(missingByKey.get(key))};`

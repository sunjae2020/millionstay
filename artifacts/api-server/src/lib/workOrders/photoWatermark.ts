 ...
 */
/**
 * 워터마크 글자 크기(px). 사진 폭에 따라 늘였다 줄였다 하면 사진마다 다르게
 * 보여서, 한 값으로 고정한다. 띠 폭을 넘치면 `c_fit`이 줄을 바꾼다 — 두세 줄이
 * 되어도 괜찮다는 게 현장 요청이다.
 */
const WATERMARK_FONT_SIZE = 45;

/**
 * 업로드된 사진에 워터마크를 얹은 배포 URL.
 *
 * 폰트는 `Arial`로 요청하지만 Cloudinary가 한글 글리프를 CJK 폰트로 대체해
 * 그린다(실측 확인). 띠 너비만 **업로드된 실제 가로폭 기준**으로 잡는 게 핵심
 * — 고정 픽셀로 주면 텍스트가 사진보다 넓어질 때 캔버스가 늘어나 사진 양옆에
 * 회색 여백이 생긴다. 여러 줄이 되면 가운데로 모아 찍는다.
 */

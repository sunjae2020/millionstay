...
export default function DocumentsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await apiGet<{ data: Doc[] }>("/v1/agent/documents");
      setRows(res.data ?? []);
    } catch (e) {
  ...
  async function onFile(file: File) {
    ...
    try {
      ...
      const res = await apiFetch("/v1/agent/documents", { method: "POST", body: fd });
      if (!res.ok) {
  ...
  async function download(doc: Doc) {
    setBusyId(doc.id);
    try {
      const res = await apiGet<{ data: { url: string } }>(`/v1/agent/documents/${doc.id}/download`);
  ...
  async function remove(doc: Doc) {
    ...
    setBusyId(doc.id);
    try {
      const res = await apiFetch(`/v1/agent/documents/${doc.id}`, { method: "DELETE" });
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {error && (
          <div className="flex items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
            ...
            <button onClick={() => setError("")} aria-label={t("common.dismiss")}><X className="w-4 h-4" /></button>
          </div>
        )}

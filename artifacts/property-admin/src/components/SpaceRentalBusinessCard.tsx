 ...
 * 공간 상세 → "임대사업자 등록" 카드.
 *
 * Settings → Organisation → 임대사업자 등록증에 등재된 세대라면, 이 호실이 등록된
...
export function SpaceRentalBusinessCard({ spaceId }: { spaceId: number }) {
  ...
  const { data } = useQuery<{ data: RegisteredUnit[]; meta?: { registration?: Registration } }>({
  ...
  if (!unit) return null;
  const registration = data?.meta?.registration ?? {};
  ...
  return (
    <div className="col-span-2 bg-card rounded-lg border p-5 flex flex-col gap-4">
      ...
      <Link href="/settings/organisation" className="text-xs text-primary underline underline-offset-2 self-start">

...
export function PropertyPhotoManager({ propertyId }: PropertyPhotoManagerProps) {
  ...
  async function fetchImages() {
    setLoading(true);
    try {
      const res = await apiFetch(`/api/v1/properties/${propertyId}/images`);
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const res = await apiFetchMultipart(`/api/v1/properties/${propertyId}/images`, formData);
      if (res.ok) {
        setPendingFiles([]);
  ...
  async function handleSetPrimary(imageId: number) {
    ...
    try {
      await apiFetch(`/api/v1/properties/${propertyId}/images/${imageId}/set-primary`, { method: "PATCH" });
      await fetchImages();
    } finally {
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 p-6 text-muted-foreground text-sm">
        <Loader2 className="h-4 w-4 animate-spin" /> {t("property_photos.loading")}
      </div>
    );
  }
    ...
    <div className="max-w-4xl space-y-6">
      ...
      {pendingFiles.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-blue-800">{t("property_photos.ready_count", { count: pendingFiles.length })}</p>
      ...
      {images.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground text-sm">
          아직 등록된 사진이 없습니다. 위 영역에 끌어다 놓거나 클릭해서 사진을 추가하세요.
        </div>
      ) : (
              ...
              <div className="relative aspect-[4/3] bg-slate-100 overflow-hidden">
                ...
                {image.is_primary && (
                  <div className="absolute top-2 left-2 flex items-center gap-1 bg-primary text-white text-xs font-medium px-2 py-0.5 rounded-full">
                    <Star className="h-3 w-3 fill-white" /> {t("property_photos.primary")}
                  </div>
                )}
              </div>
                ...
                <div className="p-3 space-y-3">
                  <div className="flex gap-1.5">
                    <Input
                      ...
                      placeholder={t("property_photos.caption_placeholder")}

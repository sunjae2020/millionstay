            ...
            <tbody className="divide-y">
              {isLoading ? (
                ...
                pagination.paginatedItems.map((policy) => (
                  <tr key={policy.id} className={`hover:bg-muted/30 transition-colors ${selectedIds.has(policy.id) ? "bg-primary/5" : ""}`}>
                    {isSuperAdmin && (
                    ...
                    <td className="px-4 py-3 font-medium">
                      <Link href={`/property/space-policies/${policy.id}`} className="hover:underline text-primary">{policy.name}</Link>
                    </td>

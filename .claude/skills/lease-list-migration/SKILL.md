...
입주일이 시트마다 하루이틀 다르게 적힌 경우(23.11.30 vs 23.12.01)도 **같은 호실 +
같은 세입자 + 7일 이내**면 한 계약으로 합친다.

**병합 방향**: 기본은 백필 — 기존 계약의 값은 건드리지 않고 **비어 있는 칸만** 채운다.
과거 연도 시트가 최신 값(연장된 종료일, 바뀐 납입일, 자동연장 계약의 빈 종료일)을
덮어쓰지 않게 하기 위함이다. 시트가 최신·권위 자료일 때만 `--overwrite` 로 시트 값을
우선한다.
...
> 과거 연도를 넣은 뒤에는 **가장 최신 연도 시트를 `--overwrite` 로 한 번 더** 돌려두면
> 최신 값이 확실히 정본으로 남는다(2023~2025 이관 후 2026 시트로 복원한 방식).
...
- **테스트 데이터 정리 시 인보이스도 함께** 소프트삭제해야 한다(계약만 지우면 재무
  대시보드 수치가 부풀려짐).
- **오래된 시트가 최신 값을 덮어쓰지 않게** 기본이 백필 모드다. 예전에 자동연장(종료일
  없음) 계약 8건이 과거 시트의 임대기간으로 종료일이 채워져 선청구 40건이 기간 밖으로
  빠진 적이 있다 — 검증 5·7번이 이 사고를 잡아낸다.

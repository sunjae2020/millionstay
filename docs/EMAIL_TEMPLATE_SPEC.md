...
`kind='sms'` 24종. **`kind` 에 CHECK 제약이 없어 마이그레이션 없이 저장된다.**

### 왜 이메일과 다른가

| | 이메일 | SMS |
|---|---|---|
| 로케일 | 6개국어 | **`ko` 하나** — 국내 발송 전용 |
| 길이 | 제한 없음 | **90바이트(한글 45자)** 넘으면 LMS·요금 3배 |
| 본문 | HTML(셸 클래스) | 평문. 태그 금지 |
| 제목 | 필수 | 없음 |
| 광고 표기 | `(광고)` + 수신거부 **링크** | `(광고)` + **무료거부 080 번호** (링크 불가) |

### 길이 검증 — 리터럴이 아니라 표본값으로 잰다

`{{변수}}` 는 발송 시점에 값이 된다. 리터럴 길이로 재면 실제 발송분이 LMS 로 넘어가도
통과한다. `_sms.mjs` 의 `SMS_SAMPLES` 가 **현실적인 최대 표본값**을 정의하고, 치환한 뒤
EUC-KR 바이트(한글 2·ASCII 1)로 판정한다. 실제로 이 검증이 초안 5건을 잡았다.

현재 24종 전부 SMS 등급(최대 88B / 평균 78B). LMS 를 의도한 템플릿은 `allowLms: true` 로
의도를 밝혀야 통과한다.

### SMS 로 보낼 것 / 보내지 않을 것

보낸다 — ① 오늘·내일 안에 사람이 움직여야 하는 일 ② 이메일을 못 볼 상황(이동 중·현장)
③ 돈이 걸린 것(납부·연체·정산) ④ 인증번호.
보내지 않는다 — 리포트·만족도·소식지. 이메일이 맞다.

### 담지 않는 것 (검증기가 차단)

- 금융·신원 정보(계좌·카드·주민번호) — 문자는 전달·캡처가 쉽고 단말에 남는다
- 리터럴 URL — 단축 링크 변수만. 원본 URL 하나가 SMS 한도를 먹는다
- `(광고)`·수신거부 문구 — 발송 코드가 붙인다. 문안에 있으면 이중 표기
- 세입자 개인정보 — 호스트 작업 지시는 **세대 호수와 출입 방법까지만**

### 🚨 발송 전에 갖춰야 할 것 (미구현)

| 항목 | 상태 |
|---|---|
| SMS 사업자 연동 (solapi·알리고 등) | **없음** — 문안은 저장되나 보낼 수단이 없다 |
| 발신번호 사전등록 (전기통신사업법) | 미확인 |
| 광고 SMS 무료거부 080 번호 | 미확보 — **확보 전에는 광고성 SMS 발송 금지** |
| 수신동의 | `marketing_consents.channel='sms'` 로 이미 지원 |

> 국내 실무에서는 **카카오 알림톡**이 SMS 보다 싸고 도달률이 높다. 붙인다면 같은 문안을
> 템플릿 심사에 올리고, 실패 시 SMS 로 대체 발송하는 구성이 된다.

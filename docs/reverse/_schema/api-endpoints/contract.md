...
| → | [`finance-payments.md` B1/B2/B3/B4](./finance-payments.md) | this → finance | `beneficiaries.contract_product_id` references `contract_products.id` (no `references()` per CF-003). The contract-products table owned by this domain is consumed by the beneficiaries domain for fee-split routing — added during T002.2.b half-2 atomic commit. |

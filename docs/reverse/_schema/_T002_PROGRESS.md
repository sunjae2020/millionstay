...
| T002.4 | ✅ | `erd-core.md` (8 cluster Mermaid + cross-cluster overview + 10 polymorphic enumeration + 73 권장 FK + 5 DEAD; CF-003 + CF-009 evidence expansion; R-REPO-9 첫 차단 게이트 가동; R-REPO-6 9회째) | 613 | **`9f51b69`** |
| T002.5 | ✅ | `state-machines.md` (5 entity Mermaid `stateDiagram-v2` + transition table + R-REPO-6 비교 column + Stripe sub-section §4.5 + cross-pack ranking matrix §7 + Booking "Pending" outlier §8 §X.fix F7 + R-REPO-7 5 결정 §9 + 35-cell self-check §10 + 3 spot-check + atomic carrier impact §11; **CF-010 본문 재작성** = T001.5 8-누락-transition 가설 → Stripe payment lifecycle (audit-only) vs invoice document lifecycle (5-state) 분리 ground truth + Phase 2 Option A/B/C 영구 보존 (B 추천 = `payment_events` 별도 entity); 6 CF evidence expansion (CF-008 5-entity audit-coverage matrix invoices 80% > contracts 71% > bookings 67% > work_orders 0% = cs_tickets 0% / CF-014 contracts.TR3 fallback `db.delete(WHERE contract_id=?)` Phase 2 footgun / CF-019.a invoices.stripe_payment_intent_id 0 write site 검증 강화 / CF-022 5-entity gated-discipline cross-pack ranking bookings 77.8% leader / CF-023 3 booking_ref generators state-entry context 매핑); **R-REPO-9 차단 게이트 2회째 가동 성공** = 사용자 5-entity 안 invoice 5-state 5/5 가짜 (Paid 1만 우연 일치) + contracts 4→7 정정 + work_orders half-true 정정 → corrected 5-entity 안 (모두 ⓑ) 사용자 채택 + CF-010 본문 재작성 동시 발주 + F7 신규 incidental 채택 = R-REPO-6 10회째; F7 신규 incidental (`guest-portal.ts:160` `booking_status="Pending"` + `:162` `status="Active"` 컬럼 미존재 → guest-portal C0' booking dead-end state 진입 admin transition 모두 거부) + F8 추가 incidental memo (cs_tickets `Resolved`/`Closed` state 부재); 0 NEW CF promotion; counts P0=4 / P1=18 / P2=3 = **25** unchanged from T002.4) | 541 | **`b0f1342`** |
| 🎯 **T002 GROUP COMPLETE** | ✅ | T002.0 ~ T002.5 전 16 doc files in `_schema/` 모두 완료 (11 endpoint domain + INDEX + SCHEMA_FILE_TABLE_MAP + db-schema-overview + erd-core + state-machines). Final CF P0=4 / P1=18 / P2=3 = **25** + 4 incidentals (F4/F5/F7/F8) + R-REPO-9 차단 게이트 2회 가동 + R-REPO-6 10회 가동 누적. Next: T003 `_context/` (자동 시작 금지) | — | (T002 group total 11 sub-task commits) |

...
시스템 로그의 IP 만으로는 "이 접속이 평소와 다른가"를 알 수 없어서 목록의 IP 컬럼과
상세 팝업에 예상 지역을 함께 보여 준다.

- 값은 외부 조회 서비스에서 받는다. 기본은 `https://ipwho.is/{ip}`(키 없음, HTTPS).
  `IP_GEO_API_URL` 로 바꾸고 `IP_GEO_LOOKUP=0` 으로 끌 수 있다(끄면 캐시에 있는 값만 뜬다).
- **밖으로 나가는 것은 IP 주소 하나뿐**이다. 사용자·계정·경로는 보내지 않는다.
  사설·루프백·CGNAT 주소는 조회 자체를 하지 않고 `status='private'` 로만 남긴다.
- 결과는 `ip_geo_cache`(마이그레이션 `0086`)에 캐시한다. 실패한 조회도 남겨서 같은 IP 를
  화면 열 때마다 다시 두드리지 않는다(재시도는 24시간 뒤). 한 요청에서 새로 조회하는 IP 는
  최대 20개, 동시 5개, 건당 2.5초 제한이다. 조회가 실패해도 목록은 그대로 나간다.
- 국가 이름은 화면에서 `Intl.DisplayNames` 로 현재 언어에 맞춰 옮긴다(6개 로케일 자동).
  도시 이름은 조회 서비스가 준 표기를 그대로 쓴다.

**어디까지나 예상이다.** VPN·모바일 캐리어·회사 프록시를 거치면 실제 위치와 다르다.
화면에도 그렇게 적어 둔다.
...
마이그레이션 `0085_user_activity_log.sql` 과 `0086_ip_geo_cache.sql` 은 **추가 전용**

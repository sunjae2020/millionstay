...
붙인 곳 — 사진이 오갈 수 있는 업로드 지점 **전부**:

- **어드민**: 공간·건물 사진, 하자(건별), 작업지시 사진, 서비스 결과 사진(2곳),
  명함·신분증(2400px), 범용 서류함(EntityDocuments — 계약·계정·연락처·공간·작업지시·거래),
  계약 서류함, 계정 서류함, 서류 일괄 인박스, CS 답변, 세대점검 증빙, 미디어 라이브러리,
  판매 매물(대표·갤러리), 사이트 콘텐츠 이미지, 사용자 사진·신분증, 회사 서류함.
- **파트너 포털 3종**: 지원 문의(작성·상세), 서류함, 작업 사진, 오너 사이트 이미지.
- **게스트 웹**: 문의(작성·상세), 서류 제출(행별), 입주 신청 증명사진, 세대점검 증빙,
  프로필 사진, 홈스테이 호스트 서류(요구 서류·일반 서류).

제외한 곳과 이유: 로고·파비콘·도장(브랜드 자산 — PNG/SVG 를 찍을 일은 없다),
CSV 임포트, 폴더 통째 업로드, 지식베이스(PDF·텍스트).
